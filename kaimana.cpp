...
void Kaimana::setLED(int index, int iR, int iG, int iB, float BRIGHTNESS)
{
  // set led identified by index to the RGB color passed to this function
 // Might have to do something like (int) floor(iR*BRIGHTNESS);
  _led[index].r = (int) floor(iR*BRIGHTNESS);
  _led[index].g = (int) floor(iG*BRIGHTNESS);
  _led[index].b = (int) floor(iB*BRIGHTNESS);
  
  _led[index+ 1].r = (int) floor(iR*BRIGHTNESS);
  _led[index+ 1].g = (int) floor(iG*BRIGHTNESS);
  _led[index+ 1].b = (int) floor(iB*BRIGHTNESS);
